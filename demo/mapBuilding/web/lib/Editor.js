...
OpenLayers.Editor = OpenLayers.Class({
    ...
    initialize: function (map, options) {
        ...
        if(!options.styleMap){
			...
			var lineStyle_default = new OpenLayers.Style({
                    fillColor: '#000',
                    fillOpacity: 1,
                    strokeColor: '#000',
                    strokeWidth: 2,
			...
			var lineStyle_select = new OpenLayers.Style({
					strokeDashstyle: 'dash'
                });
			var lineStyle_temporary = new OpenLayers.Style({
					fillOpacity: 0.8,
					strokeDashstyle: 'dash'
                });
			lineStyle_default.addRules([
			new OpenLayers.Rule({
				symbolizer: linemeasureStyles
			})]);
			lineStyle_select.addRules([
			new OpenLayers.Rule({
				symbolizer: linemeasureStyles
			})]);
			lineStyle_temporary.addRules([
			new OpenLayers.Rule({
				symbolizer: linemeasureStyles
			})]);
			this.editLayer.styleMap = new OpenLayers.StyleMap({
				"default": lineStyle_default,
				"select": lineStyle_select,
				"temporary": lineStyle_temporary
			});
        }
